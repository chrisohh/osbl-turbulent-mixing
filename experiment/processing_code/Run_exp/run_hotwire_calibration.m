...
FAN_V_LEVELS   = [2 2.1 2.2 2.5 3.0];   % V, ascending
...
STEP_SETTLE_T  = 0;    % s, discarded after each step change (flow settling)
...
if hasRef
    ...
    fprintf('Reference probe (cert %s, %s): %.4f..%.4f V  ->  %.3f..%.3f m/s\n', ...
        cal_ref.id, cal_ref.date, min(E_ref), max(E_ref), min(U_ref), max(U_ref));
    % Clamped samples are floor/ceiling values, not measurements -- a step
    % whose average is built from them is not a usable calibration point.
    if cal_ref.nAbove > 0
        warning('Flow exceeded the reference probe''s calibrated range (%.2f m/s).', ...
            cal_ref.U_max);
    end
end
